...
const options = {
  renderMark: {
    ...
    [MARKS.CODE]: (text) => (
      <pre
        className="text-gray-600 bg-gray-300 px-2 py-1 rounded mb-0"
  ...
  renderNode: {
    ...
    [BLOCKS.LIST_ITEM]: (node, children) => (
      <li className="md:pl-2">{children}</li>
    ),
    [BLOCKS.UL_LIST]: (node, children) => (
      <ul className="pl-12 md:pl-16 list-disc mb-12 max-w-3xl mx-auto ">
        {children}
      </ul>
    ),
    [BLOCKS.OL_LIST]: (node, children) => (
      <ol className="pl-12 md:pl-16 list-decimal mb-12 max-w-3xl mx-auto">
        {children}
      </ol>
    ),
    ...
    [INLINES.EMBEDDED_ENTRY]: (node, children) => {
      if (
        node.data.target.sys.contentType.sys.contentful_id === "inlinePostPhoto"
      ) {
        // This stupid object for determining if there is an image at the end
        if (
          node.data.target.fields.photo["en-US"].fields.file[
            "en-US"
          ...
          return (
            <RichInlineImage image={image} className="max-w-3xl my-8 mx-auto" />
          )
        }
      }
    },
